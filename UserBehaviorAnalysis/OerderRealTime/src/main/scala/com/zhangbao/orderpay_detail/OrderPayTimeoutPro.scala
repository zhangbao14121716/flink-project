...
object OrderPayTimeoutPro {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val timeoutOutputTag: OutputTag[OrderPayResult] = new OutputTag[OrderPayResult]("timeoutOutSideStream")
    //
    val resultStream: DataStream[OrderPayResult] = patternStream
      .select(timeoutOutputTag, new OrderPayTimeoutSelect(), new OrderPaySelect())
...
class OrderPayTimeoutSelect() extends PatternTimeoutFunction[OrderEvent, OrderPayResult]{
  ...
  override def timeout(pattern: util.Map[String, util.List[OrderEvent]], timeoutTimestamp: Long): OrderPayResult = {
    ...
    OrderPayResult(timeoutOrderId, s"15分钟没有支付的订单 ,time out at ${timeoutTimestamp}")
