package com.zhangbao.network_flow.API_3_window_watermark_otherAPI
 ...
 * @Description:
 * 窗口API演示，防止数据丢失三道防线
 * 1.Watermark（基于DataStream）-防止分布式带来的数据乱序延迟（乱序抖动很小）
 * 2.allowedLateness(允许最大等待时间),（基于WindowedStream）-保证迟到数据（实现lambda架构）
...
object WindowTest {
  def main(args: Array[String]): Unit = {
    ...
    val aggStream: DataStream[SensorReading] = dataStream
      ...
      //.trigger()
      //.evictor()
      .allowedLateness(Time.minutes(1)) //延迟等待时间
